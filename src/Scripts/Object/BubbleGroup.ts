...
export default class BubbleGroup {
    ...
    for (let i = 0; i < this.maxRow; i++) {
      ...
      for (let j = 0; j < (i % 2 ? sizeX - 1 : sizeX); j++) {
        ...
        if (i < sizeY) {
          ...
          let newBubble = this.createBubble(
            j * 85 + (i % 2 ? 110 : 70),
            i * 75 + 100,
            j,
            i,
  ...
  collide(overlappedBubble, playerBubble: Bubble): void {
    ...
    if (
      playerBubble.y >= (getResolution().height * 4) / 5 - 120 ||
  ...
  getClosestNeighbor(bubbleA, bubbleB) {
    const offset = this.isLongRowFirst
      ? bubbleA.getIndex().y % 2
      : (bubbleA.getIndex().y + 1) % 2;
    ...
    neighborsoffsets[offset].forEach((neighborOffset) => {
      ...
      if (
        ...
        x >= 0 &&
        y >= 0 &&
        x < this.getRow(y).length
      ) {
        let position = this.calculatePosition(x, y);
        let distance =
          Math.abs(bubbleBPos.y - position.y) +
          Math.abs(bubbleBPos.x - position.x);
    ...
    return {
      x: bubbleA.getIndex().x + nearestNeighbor[0],
      y: bubbleA.getIndex().y + nearestNeighbor[1],
    };
  }
  ...
  calculateIndex(posX, posY): any {
    ...
    let x = Math.floor(
      (posX - (this.isLongRowFirst ? (y % 2 ? 40 : 0) : y % 2 ? 0 : 40)) / 85
    );
    return { x: x, y: y };
  }
  ...
  calculatePosition(x, y): any {
    const posX =
      x * 85 + (this.isLongRowFirst ? (y % 2 ? 110 : 70) : y % 2 ? 70 : 110);
    const posY = y * 75 + 100;
  ...
  addRow(): void {
    ...
    for (
      let i = 0;
      i < (this.isLongRowFirst ? this.sizeX : this.sizeX - 1);
      i++
    ) {
  ...
  checkAround(
    ...
    neighborsoffsets[offset].forEach((neighborOffset) => {
      let currentBubble = this.getBubble(
        indexX + neighborOffset[0],
        indexY + neighborOffset[1]
      );
      if (
        currentBubble &&
        ((colorCode !== null && currentBubble.getColorCode() === colorCode) ||
          colorCode === null) &&
        !currentBubble.getProcessed() &&
        !currentBubble.getIsPopped()
      ) {
  ...
  checkAroundNull(indexX, indexY, groupCopy, bubbleNeighbor): any {
    ...
    neighborsoffsets[offset].forEach((neighborOffset) => {
      ...
      if (
        currentIndexY >= 0 &&
        currentIndexY < this.maxRow &&
        currentIndexX < groupCopy[currentIndexY].length &&
        currentIndexX >= 0
      ) {
        let currentBubble =
          groupCopy[indexY + neighborOffset[1]][indexX + neighborOffset[0]];
        // this.getBubble(indexX + neighborOffset[0], indexY + neighborOffset[1]);
        if (currentBubble === null) {
          neighbors.concat(
            this.checkAroundNull(
              indexX + neighborOffset[0],
              indexY + neighborOffset[1],
              groupCopy,
              bubbleNeighbor

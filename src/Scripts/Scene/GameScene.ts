...
export default class GameScene extends Phaser.Scene {
      ...
      function (pointer) {
        if (!this.gameOver) {
          ...
          if (angle > 87) {
            ...
            this.guideLine.onPointerMove({
              x: getResolution().width / 2,
              y: 0,
            });
          } else {
  ...
  runGameOver(): void {
    ...
    this.add
      .text(
        getResolution().width / 2,
        getResolution().height / 3,
        this.win ? "You win" : "Game Over",
        { fontSize: "45pt", color: "#ffffff" }
      )
      .setOrigin(0.5, 0.5);
    let restartButton = this.add.sprite(
      getResolution().width / 2,
      (getResolution().height * 3) / 4,
      "replay"
    );
  ...
  nextMove(): void {
    this.currentBubble.setColor(this.nextBubble.getColorCode());
    this.nextBubble.randomizeColor(
      this.bubbleGroup.getReachableCluster(),
      true
    );
  }
